...
export default function Press() {
  ...
  const routeBasePath = routeSite ? `/${routeSite}` : "";
  const pressBasePath = routeSite ? `${routeBasePath}/press` : "/Promotion/Press";
  const pressAdminPath = routeSite
    ? `${routeBasePath}/press/admin`
    : "/Promotion/Press/Admin";
  const customerPath = routeSite
  ...
  const seoPage = useMemo(
    () => ({
      ...
      description:
        "고덕 제일풍경채 언론보도 페이지입니다. 평택 고덕국제신도시 Abc-14BL·Abc-61BL 공급 정보, 청약, 입지환경, E-모델하우스와 방문예약 관련 공식 보도자료와 분양 소식을 확인하세요.",
      extraSchemas: [
        {
          "@context": "https://schema.org",
          "@type": "ItemList",
          "@id": `${getAbsoluteUrl(pressBasePath)}#press-list`,
          name: "고덕 제일풍경채 언론보도 목록",
        ...
        <section className={styles.pressHero} aria-labelledby="pressTitle">
          <div>
            ...
            <p className={styles.pressLead}>
              평택 고덕국제신도시 Abc-14BL·Abc-61BL 공급 정보부터 청약,
              입지환경, E-모델하우스와 방문예약까지 고덕 제일풍경채의 공식
              보도자료와 분양 소식을 전합니다.
            </p>
          ...
          <div className={styles.pressStats} aria-label="고덕 제일풍경채 주요 정보">
            <div>
              <strong>{siteSeo.project.households || "1,126세대"}</strong>
              <span>공급 규모</span>
            </div>
            <div>
              <strong>{siteSeo.project.block || "Abc-14BL · Abc-61BL"}</strong>
              <span>사업 위치</span>
            </div>
            <div>
